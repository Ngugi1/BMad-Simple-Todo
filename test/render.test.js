...
test('completed task uses plain strike-through fallback when ansi is off', () => {
  assert.equal(renderList([{ id: 1, text: 'done thing', done: true }]), '1. ~~done thing~~');
});

test('completed task uses ANSI strike-through when ansi is on', () => {
  assert.equal(
...
test('mixed open and completed tasks render on their own lines (plain)', () => {
  const out = renderList([
